...
def parse_args():
    ...
    return args
def collate_fn(examples,sample_n_frames_bucket_interval):
    args = parse_args()
...
def worker_init_fn(worker_id, base_seed):
    seed = base_seed * 256 + worker_id
    print(f"worker_init_fn with {seed}")
    np.random.seed(seed)
    random.seed(seed)
def main():
    args = parse_args()
    ...
    if args.enable_bucket:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            ...
            collate_fn=partial(collate_fn,sample_n_frames_bucket_interval=sample_n_frames_bucket_interval),
            persistent_workers=True if args.dataloader_num_workers != 0 else False,
            num_workers=args.dataloader_num_workers,
            # worker_init_fn=worker_init_fn(args.seed + accelerator.process_index)
            worker_init_fn=partial(worker_init_fn, base_seed=args.seed + accelerator.process_index)
        )
    else:
        # DataLoaders creation:
        batch_sampler_generator = torch.Generator().manual_seed(args.seed)
